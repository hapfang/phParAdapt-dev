...
readErrorFromRestart(double* nodalErrors, int stepNumber){
    ...
    int restart;
    char rfname[40];
    sprintf(rfname,"restart.%d.%d",stepNumber,PMU_rank()+1);
    ...
    readheader_( &restart, "error", iarray,
    ...
    if(readStepNum != stepNumber ){
        if(PMU_rank() == 0){
            ...
            cerr<<"\nerror in readErrorFromRestart.cc: stepNumber retrieved differs\n"
    ...
    readdatablock_( &restart, "error", qlocal, &isize,
                    "double" , iformat );
