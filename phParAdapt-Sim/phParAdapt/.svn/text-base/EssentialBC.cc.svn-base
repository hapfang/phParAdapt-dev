...
// old extern "C" pAttribute Attribute_childByType(pAttribute att, const char *type);

// replaces Simmetrix functions
...
enum { thermo, velo, scalar, axisym };  // to set dontinherit accordingly

#define numAttsE 10                       // number of essential bc attributes
enum { D, T, P, C1, C3, S1, S2, S3, S4, TBI };//for readability of essential stuff
...
//     try {
//       atts = SCOREC_att::retrieveAttributePList((pGEntity)gface, strAttE[i]);
//     } catch (AttributeNotExistent) {}
//     if ( atts.size() >= 1 ) loop = false ;
//     atts.clear();
// #endif
...
// #else
//       try {
//         std::vector<Attribute*> atts1 = SCOREC_att::retrieveAttributePList((pGEntity)gface, strAttE[i]);
//         AttList[i] = atts1[0];
//         atts1.clear();

//       } catch (AttributeNotExistent) {AttList[i] = 0 ; }

//       /* assuming only one attribute of this type exists */
// #endif
    }
  }
}
...
// #else
//     try {
//       std::vector<Attribute*> atts = SCOREC_att::retrieveAttributePList((pGEntity)gedge, strAttE[i]);
//       AttList[i] = atts[0];
//       atts.clear();
//     } catch (AttributeNotExistent) {AttList[i] = 0 ; }
...
// #else
//     try {
//       std::vector<Attribute*> atts = SCOREC_att::retrieveAttributePList((pGEntity)gvert, strAttE[i]);
//       AttList[i] = atts[0];
//       atts.clear();
//     } catch (AttributeNotExistent) {AttList[i] = 0 ; }
...
int EssentialBC::evalE(pEdge edge, double *BC, int p)   // hierarchic basis
{
...
int EssentialBC::evalF(pFace face, double *BC, int p)
{
  // find centroid of face
  double xyz[3][3];
#ifdef SIM
  F_coord(face,xyz,NULL);
#endif
#ifdef FMDB
  F_coord(face,xyz);
#endif
...
void EssentialBC::update_velo(const double *x, double *BC, int *ibcp) // private
{
  ...
  if ((static_cast<bool>(n=C1) && isAttSet(C1)) || (static_cast<bool>(n=C3) && isAttSet(C3)))  {
//old    Att_evalTensorOr1(Att_childByType(AttList[n],"vector direction"), x, BC+3);

//      Att_evalTensorOr1(Attribute_childByType(AttList[n],"vector direction"), x,
//                        BC+3);
    ...
    BC[4] /= mag;
    BC[5] /= mag;
//ol old    BC[6] = Att_evalTensorOr0(Att_childByType(AttList[n],"vector magnitude"),x);
//old     BC[6] = Att_evalTensorOr0(Attribute_childByType(AttList[n],"vector magnitude"),x);
    BC[6] = AttributeDouble_evalDS ( (pAttributeDouble)Attribute_childByType( AttList[n], "vector magnitude"),
                                      const_cast<double*>(x));
...
{
//old
//    if (isAttSet(D))  BC[0] = Att_evalTensorOr0(AttList[D], x);
//    if (isAttSet(T))  BC[1] = Att_evalTensorOr0(AttList[T], x);
//    if (isAttSet(P))  BC[2] = Att_evalTensorOr0(AttList[P], x);
...
void EssentialBC::update_scalar(const double *x, double *BC, int *ibcp)
{
  ...
  int nscalar=ensa_dof - 5;

  for(i=0; i<nscalar; i++){
    if (isAttSet(S1+i))
//old       BC[12+i] = Att_evalTensorOr0(AttList[S1+i], x);
       BC[12+i] = AttributeTensor0_evalDS((pAttributeTensor0)AttList[S1+i], const_cast<double*>(x));
...
{
  int i, tbit, vbit, sbit,abit;  // whether thermo, velo or scalar is supposed to be inherited
            ...
            if (uset && vset && wset)  {               // all 3 set
              ...
              *ibcp = setbit(*ibcp, n);
              ...
              if (getbit(*ibcp,p))  {     // ?? wont check V2 BC's if all 3 set
                ...
                for (i=0; i<3; i++)  v[i] = ValueOnFace[i+3]*ValueOnFace[6] + ValueOnFace[i+7]*ValueOnFace[10];
