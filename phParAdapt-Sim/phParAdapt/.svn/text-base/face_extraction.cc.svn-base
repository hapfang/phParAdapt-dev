...
extern pVertex F_oppositeVertex(pFace face, pEdge edge);
...
std::map<pFace , int > counted;
/* List of vertices on the front bdry */
std::map<pEntity , int > vertex_list;
/* List of edges on the front bdry */
std::map<pEntity , int > edge_list;

/* List of deleted entities */
std::map<pEntity, int > dvertex;
...
//      pEdge edgy;
//      while(edgy = EIter_next(eIter)) E_setWhatIn(edgy,(pGEntity)gface[0]);
//      EIter_delete(eIter);
//
//      /* reclassifying the faces */
...
//
//  std::map<pEntity, int >::iterator edgeiter;
...
////old        vertex = F_edOpVt(candidate ,(pEdge)((*edgeiter).first));
...
//
//               numchanges++;
////old              nuke[2] = (pEntity)F_edOpVt(candidate,medge_candidate);
...
//  std::vector<MFace *>::iterator cvIter;
//  pRegion region;
//  int dir;
//
