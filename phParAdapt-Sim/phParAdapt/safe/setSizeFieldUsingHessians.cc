...
//   adaptSimLog<<"with max. edge length : "<<hmax<<endl;
  ...
  while ( vertex=VIter_next(vit)) {
    ...
    dir2[0]=hess[i].dir[0][0];dir2[1]=hess[i].dir[0][1];dir2[2]=hess[i].dir[0][2];
    dir2[3]=hess[i].dir[1][0];dir2[4]=hess[i].dir[1][1];dir2[5]=hess[i].dir[1][2];
    dir2[6]=hess[i].dir[2][0];dir2[7]=hess[i].dir[2][1];dir2[8]=hess[i].dir[2][2];
...
//        if (newSize < MaxRefineFactor) newSize = MaxRefineFactor;
        ...
        hess[i].h[0] = newSize*hess[i].h[0]/hess[i].h[2];
        hess[i].h[1] = newSize*hess[i].h[1]/hess[i].h[2];
        hess[i].h[2] = newSize;
...
// /* compute the size field        
  ...
  SizeQuality(mesh);
//  M_writeVTKFile(mesh,"OriginalSize",OrgSizeID,3);
