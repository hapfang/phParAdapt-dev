...
//  mesh = PM_mesh(pmesh, 0);
  ...
  if(strategy!=7 ) {
    ...
    adc = PM_newAttachDataCommu(sizeof(double)/sizeof(int),0,numVars);
    if(adc){
...
// MSA_setBLSnapping(simAdapter, 0);
    ...
    if(option>0 && strategy==1) {
      ...
      if(option == 4) {
        ...
        attachArray(ybar,mesh,ybarID,ndof,poly);
        delete [] ybar;
      }
      ...
      if(isThickAdapt){
         ...
         double *wall_stress;
         readArrayFromFile(wall_stress_file,wall_stress_tag,wall_stress);
         attachArray(wall_stress,mesh,wallStressID,6,poly);
         for(int i=0;i<10;i++){
            SmoothStress(mesh);
         }
//         DistOffWall(mesh, simAdapter);
      ...
      M_writeVTKFile(mesh,"nodalEI", errorIndicatorID, 6); 
...
//      delete [] sol;
    ...
    if(nvar){
      ...
      setManualSizeField(mesh,simAdapter,strategy);
      MSA_setCallback(simAdapter, NULL,9,NULL);
#ifdef SIM
    printf("isBLAdapt=%i\n\n",isBLAdapt);
    MSA_setAdaptBL(simAdapter, isBLAdapt); 
  ...
  if(strategy==1 || strategy==3 || strategy==4)
